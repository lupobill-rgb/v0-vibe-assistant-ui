...
export class NangoService {
  ...
  /**
   * Returns the Nango Connect URL to initiate an OAuth flow for a team.
  ...
  async getConnectUrl(
    ...
    this.logger.log(
      `Initiating OAuth — team=${teamId} connector=${connectorType}`,
    );
  ...
  /**
   * Retrieves an active connection for a team+connector pair.
  ...
  async getConnection(
    ...
    try {
      ...
      this.logger.warn(
        `No active connection — team=${teamId} connector=${connectorType}: ${(err as Error).message}`,
      );
      return null;
    }
  }
  ...
  /**
   * Deletes a connection when a team disconnects an integration.
   */
  async deleteConnection(
    ...
    this.logger.log(
      `Connection deleted — team=${teamId} connector=${connectorType}`,
    );
  }

  /**
   * Returns all ConnectorTypes that have an active connection for a team.
   */
  async listActiveConnections(teamId: string): Promise<ConnectorType[]> {
    const checks = await Promise.allSettled(
      Object.values(ConnectorType).map((ct) =>
        this.getConnection(teamId, ct),
      ),
    );
    ...
    return Object.values(ConnectorType).filter(
      (_, i) =>
        ...
        (checks[i] as PromiseFulfilledResult<NangoConnection | null>).value !==
          null,
    );
  }
}
