 ...
 */
async function markComplete(executionId: string, result: Record<string, unknown>): Promise<void> {
  const sb = getPlatformSupabaseClient();
 ...
 * 3. Call the edge function with the skill + trigger payload
...
async function executeOne(exec: AutonomousExecution): Promise<void> {
  ...
  if (!skill) {
    const msg = `Skill ${exec.skill_id} not found or inactive`;
