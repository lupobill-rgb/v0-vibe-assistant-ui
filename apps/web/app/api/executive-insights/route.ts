...
async function callLLM(messages: Array<{ role: string; content: string }>, system: string) {
  ...
  if (deepseekKey) {
    ...
    console.warn("[executive-insights] DeepSeek failed, trying Anthropic:", data.error?.message)
  }
  if (!anthropicKey) throw new Error("No LLM API key configured")
