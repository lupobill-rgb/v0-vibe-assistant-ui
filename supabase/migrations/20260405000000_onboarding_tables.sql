-- Migration: Enterprise onboarding tables
-- Creates onboarding_sessions, onboarding_steps, onboarding_connectors, org_feature_flags
-- Idempotent: all operations use IF NOT EXISTS / ON CONFLICT DO NOTHING

-- ============================================================================
-- 1. org_feature_flags — controls per-org stage routing (e.g. onboarding)
-- ============================================================================
...
CREATE POLICY org_feature_flags_select ON org_feature_flags
...
CREATE POLICY org_feature_flags_update ON org_feature_flags
...
-- ============================================================================
-- 2. onboarding_sessions — one per organization
-- ============================================================================
...
-- ============================================================================
-- 3. onboarding_steps — one row per step per session (seeded on session create)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_onboarding_steps_session
...
CREATE POLICY onboarding_steps_select ON onboarding_steps
  ...
  USING (
    session_id IN (
      ...
      JOIN org_members om ON om.org_id = os.organization_id
...
-- ============================================================================
-- 4. onboarding_connectors — tracked per session
-- ============================================================================
CREATE TABLE IF NOT EXISTS onboarding_connectors (
  ...
  status TEXT NOT NULL DEFAULT 'available'
    CHECK (status IN ('available', 'connecting', 'connected', 'failed', 'skipped')),
...
CREATE INDEX IF NOT EXISTS idx_onboarding_connectors_session
  ON onboarding_connectors(session_id);

ALTER TABLE onboarding_connectors ENABLE ROW LEVEL SECURITY;

CREATE POLICY onboarding_connectors_select ON onboarding_connectors
  FOR SELECT TO authenticated
  USING (
    session_id IN (
      SELECT os.id FROM onboarding_sessions os
...
CREATE POLICY onboarding_connectors_update ON onboarding_connectors
  FOR UPDATE TO authenticated
  USING (
    session_id IN (
      SELECT os.id FROM onboarding_sessions os
